...
### 积分池：平台把积分拆成多个池时必须逐池声明

有些平台把积分拆成多个**独立计额**的池，例如「专属积分池」和「通用积分池」，
各自有独立的 5 小时和周额度。规则通常是**先用专属池，专属不够再扣通用池**。

危险之处在于：专属池用尽时平台**不报错**，而是静默改从下一个池扣费，
请求照样返回 200。工具察觉不到，继续发，就会把不该动的池吃掉——
而被动扣掉的那部分通常不参与返赠活动。

在「④ 额度与时长 → 积分池」里逐池声明，工具就会按池核算，某个池用满时停手：

| 池名称 | 匹配模型 | 5 小时上限 | 周上限 |
|---|---|---|---|
| Flash-Lite 专属积分 | `*flash-lite*` | 60000 | 600000 |
| 通用积分 | `*` | 60000 | 600000 |

按顺序匹配，**首个命中生效**。所以专属池写前面、兜底的 `*` 写最后，
语义就和平台「专属不足再扣通用」的规则对齐。模型填 glob 模式。

留空即单池，行为等同上面的 5 小时 / 周额度。

**换算系数填错之后要重算历史。** 窗口积分是逐请求落库的，改系数**不会**自动改历史
（这是有意为之，正常调参不该篡改已发生的事实）。但系数**填错之后修正**是另一回事：
不重算的话窗口会一直显示错的占用，周窗口最长要卡 7 天，期间工具一直以为额度满了。
确认新系数填对了，再点「重算历史」。
